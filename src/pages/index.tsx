import { getflight, getflightbysearch, gethotel, hotelsByPlace, recommandFlights, recommandHotels } from "@/api";
import Loader from "@/components/Loader";
import ReccomandFlight from "@/components/ReccomandFlight";
...
export default function Home() {
  ...
  const [recommandedHotels, setRecommandedHotels] = useState<any[]>([]);
  const [recommandedFlights, setRecommandedFlights] = useState<any[]>([]);
      ...
      try {
        ...
        const [hotelData, flightData, recFlights, recHotels] =
          await Promise.all([
            gethotel(),
            getflight(),
            token ? recommandFlights() : Promise.resolve([]),
            token ? recommandHotels() : Promise.resolve([]),
          ]);

        sethotel(hotelData);
        setflight(flightData);
        setRecommandedFlights(recFlights);
        setRecommandedHotels(recHotels);
      } catch (error) {
        console.error(error);
      } finally {
        setloading(false);
  ...
  const handlesearch = async () => {
    if (bookingtype === "flights") {
      const results = await getflightbysearch(from, to);
      setsearchresult([...results]);
    } else if (bookingtype === "hotels") {
      const results = await hotelsByPlace(to);
      setsearchresult(results);
    }
  };
    ...
    >
      <main className="container mx-auto px-4 py-6">
        ...
        <div className="max-w-7xl mx-auto px-4">
          {
            bookingtype === "flights" ? (<ReccomandFlight flight={recommandedFlights} />) : (<Reccomendation hotels={recommandedHotels} />)
          }
